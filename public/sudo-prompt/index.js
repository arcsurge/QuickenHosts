...
function linux(instance, end) {
    linuxBinary(instance, function (error, binary) {
        ...
        for (let key in instance.options.env) {
            let value = instance.options.env[key]
        ...
        node.child.exec(
            ...
            function (error, stdout, stderr) {
                ...
                let elevated = stdout && stdout.slice(0, magic.length) === magic
...
function mac(instance, callback) {
    let temp = node.os.tmpdir()
    if (!temp) return callback(new Error('os.tmpdir() not defined.'))
    let user = node.process.env.USER // Applet shell scripts require $USER.
...
function macApplet(instance, end) {
    let parent = node.path.dirname(instance.path)
    node.fs.mkdir(parent, function (error) {
        if (error) return end(error)
        let zip = node.path.join(parent, 'sudo-prompt-applet.zip')
...
function macCommand(instance, end) {
    let path = node.path.join(instance.path, 'Contents', 'MacOS', 'sudo-prompt-command')
    let script = []
    // Preserve current working directory:
...
function macIcon(instance, end) {
    ...
    node.fs.readFile(instance.options.icns, function (error, buffer) {
        if (error) return end(error)
        let icns = node.path.join(instance.path, 'Contents', 'Resources', 'applet.icns')
...
function macOpen(instance, end) {
    ...
    let binary = node.path.join(instance.path, 'Contents', 'MacOS', 'applet')
...
function macPropertyList(instance, end) {
    ...
    let plist = node.path.join(instance.path, 'Contents', 'Info.plist')
    let path = escapeDoubleQuotes(plist)
    let key = escapeDoubleQuotes('CFBundleName')
    let value = instance.options.name + ' Password Prompt'
    if (/'/.test(value)) {
...
function macResult(instance, end) {
    let cwd = node.path.join(instance.path, 'Contents', 'MacOS')
...
function windowsElevate(instance, end) {
    ...
    // let command = 'cscript.exe //NoLogo "' + instance.pathElevate + '"';
    let command = []
    ...
    if (instance.options.admin) {
        command.push('-Verb runAs')
    }
    command = command.join(' ')
    let child = node.child.exec(command, { encoding: 'utf-8' }, function (error, stdout, stderr) {
...
let PERMISSION_DENIED = 'User did not grant permission.'
let NO_POLKIT_AGENT = 'No polkit authentication agent found.'
